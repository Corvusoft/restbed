...
namespace restbed
{
    namespace detail
    {
        IPCSocketImpl::IPCSocketImpl( asio::io_context& context, const shared_ptr< stream_protocol::socket >& socket, const std::string& path, const shared_ptr< Logger >& logger ) : SocketImpl( context ),
            ...
            m_strand( make_shared< asio::strand< asio::io_context::executor_type > > ( asio::make_strand( m_io_service ) ) ),
            m_socket( socket )
        {
            return;
        }
        ...
        {
            m_socket->async_connect( stream_protocol::endpoint( m_path ), [ this, self = shared_from_this(), callback ]( const error_code & error )
            {
                m_is_open = true;
                callback( error );
            } );
